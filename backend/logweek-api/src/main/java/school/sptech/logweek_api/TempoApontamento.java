...
public class TempoApontamento {
    ...
    public static LocalDate lerSemana(String valor) {
        if (valor == null || valor.isBlank()) throw new IllegalArgumentException("Semana ausente");
        if (valor.matches("\\d{4}-\\d{2}-\\d{2}")) return segundaFeira(LocalDate.parse(valor));
        if (valor.matches("\\d{2}/\\d{2}/\\d{4} - .*")) {
            return segundaFeira(LocalDate.parse(valor.substring(0, 10), DATA));
        }
        String[] partes = valor.split(" - ");
        if (partes.length != 2) throw new IllegalArgumentException("Semana invalida");
        String[] inicio = partes[0].split(" ");
        String[] fim = partes[1].split(" ");
        String[] meses = {"jan", "fev", "mar", "abr", "mai", "jun", "jul", "ago", "set", "out", "nov", "dez"};
        int mes = 0;
        for (int i = 0; i < meses.length; i++) if (meses[i].equals(inicio[1])) mes = i + 1;
    ...
    public static LocalDate semanaDaNota(Apontamento nota) {
        if (nota.getInicioEm() != null) return segundaFeira(dataHora(nota.getInicioEm()).toLocalDate());
        try { return lerSemana(nota.getSemana()); }
        catch (RuntimeException erro) { return segundaFeira(dataHora(nota.getCriadoEm()).toLocalDate()); }
    }
